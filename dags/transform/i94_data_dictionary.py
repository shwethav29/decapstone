...
def process_state_codes():
    ...
    df_state = spark.read.format("csv").option("delimiter", "=").option("header", "False").load("../../"+STATE_FILE)
...
def process_country_codes():
    ...
    df_country = spark.read.format("csv").option("delimiter", "=").option("header", "False").load("../../"+COUNTRY_FILE)
...
def process_airport_codes():
    ...
    #sinput_data_file = os.path.join(s3, I94_CODES_DATA_PATH + AIRPORT_FILE)
    df_airport = spark.read.format("csv").option("delimiter", "=").option("header", "False").load("../../"+AIRPORT_FILE)
